...
pipeline {
	...
	stages {
		...
		stage('Build') {
			steps {
				...
				sh './bin/kwmbridged version && sha256sum ./bin/kwmbridged'
			}
		}
	...
	post {
		always {
			...
			recordIssues enabledForFailure: true, qualityGates: [[threshold: 400, type: 'TOTAL', unstable: true]], tools: [checkStyle(id: 'golint', name: 'Golint', pattern: 'test/tests.lint.xml')]
